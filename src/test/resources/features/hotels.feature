...
Feature: Verify dates from date pickers
  ...
  @hotel-TA7-4
  Scenario: Verify subscription function of hotels.com newsletter
  ...
  Scenario: Verify user is able to successfully search for hotels
    When I clicked on Destination on home page
    And I insert Dallas into Destination field on home page
    And I clicked on Dallas, Texas, United States of America from autoSuggestions
    And I clicked on Check in on home page
    And I select tomorrow date from date picker
    And I clicked on Check out on home page
    And I add 6 days with current date and click on that date
    Then I verify 6 nights have been selected
    When I click the occupancy drop down menu and select More options…
    And I click the rooms drop down menu and select 1
    And I click the adult drop down menu and select 2
    And I click the children drop down menu and select 2
    And I click the child1 drop down menu and select <1
    And I click the child2 drop down menu and select 3
    And I clicked on search button on home page
  ...
  Scenario: pick dates from calendar
    When I clicked on Check in on home page
